...
**Project:** NoTrade  
**Purpose:** estimate `P(event)` at decision time and ensure that the probabilities used for ENTER / PASS decisions are honest enough to survive real-world evaluation.
...
It predicts the probability that a **specific market event** resolves YES under the exact rules defined in `MARKET RULES.md`.
...
- `probs.market_prob_yes` remains the market-implied probability from Polymarket;
...
Labels must be created strictly from the canonical rule system defined in `MARKET RULES.md`.
