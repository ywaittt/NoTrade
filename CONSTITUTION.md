...
- outputs a disciplined decision: **ENTER / PASS / HOLD / EXIT**
...
### 1.4 Canonical decision verbs
The repo uses four top-level policy verdicts:
- **ENTER** = open or add exposure
- **PASS** = do not open fresh exposure
- **HOLD** = keep an already open thesis alive without full exit
- **EXIT** = full flatten of the relevant thesis

`HOLD` is valid only when a position is already open. `EXIT` is reserved for full-exit logic; partial `SELL` operations remain transaction-level actions.
...
> Using mid-price is allowed only for reporting, not for live policy decisions.
